...
from runners.HistogramSVMRunner_kernelsvm_v3 import SpeckleRunner
#from runners.HistogramSVMRunner_kernelsvm_v4 import SpeckleRunner
...
def arg_parse():
    ...
    parser.add_argument('--bias', type=int, default=10, help='T Frames per batch')
    ...
    parser.add_argument('--cam', type=str, default='cam_0', help='Optional Ongoing Threadm information')
...
if __name__ == "__main__":
    ...
    loaders = SpeckleLoader(
            ...
            test=arg.test, flow=arg.flow, T=arg.image_T, val_sample=arg.val_sample, method=arg.method, cam=arg.cam, bias=arg.bias
    )

    num_bins = 100
    net = SVM(num_bins, labels).to(torch_device)
    net = nn.DataParallel(net).to(torch_device)
    loss = nn.CrossEntropyLoss()

    optim = {
        "adam": torch.optim.Adam(net.parameters(), lr=arg.lr, betas=arg.beta, weight_decay=arg.decay),
        "sgd": torch.optim.SGD(net.parameters(), lr=arg.lr, momentum=0)
    }["sgd"]

    model = SpeckleRunner(arg, net, optim, torch_device, loss, logger)
    if arg.test is False:
        train_loader, val_loader = loaders
        model.train(train_loader, val_loader, num_bins=num_bins, min=1e-3) #, max=3e-2)
