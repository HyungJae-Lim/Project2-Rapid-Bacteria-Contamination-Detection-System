...
#            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optim, 100)
    ...
    def load(self, filename=None):
        ...
        backbone_file_path = self.backbone_save_dir

        if os.path.exists(file_path) is True:
            print("Load %s to %s File" % (self.save_dir, filename))
            ckpoint = torch.load(file_path)
            backbone_ckpoint = torch.load(backbone_file_path)
            if ckpoint["model_type"] != self.model_type:
                raise ValueError("Ckpoint Model Type is %s" % (ckpoint["model_type"]))

            self.net.load_state_dict(ckpoint['network'])
            self.backbone.load_state_dict(backbone_ckpoint['network'])
    ...
    def train(self, train_loader, val_loader=None, test_loader=None):
        ...
        for epoch in range(self.start_epoch, self.epoch):
            ...
            for train_b_id in pbar:
                ...
                if self.arg.reverse_axis:
                    input1 = input1.permute(0, 4, 2, 3, 1)
                    input2 = input2.permute(0, 4, 2, 3, 1)
    ...
    def _get_acc(self, loader, confusion=False, test=False):
        ...
        for eval_b_id in pbar:
            ...
            # Extract contam predictions
            con_tmp1 = prediction1[prediction1 == 0]
            con_tmp2 = prediction2[prediction2 == 0]

            # GT Labels of contam predictions
            target_tmp1 = cls_target1[prediction1 == 0]
            target_tmp2 = cls_target2[prediction2 == 0]

            # Count correct contam predictions
            con_correct += torch.sum((con_tmp1 == target_tmp1).int())
            con_correct += torch.sum((con_tmp2 == target_tmp2).int())

            # Count the whole number of contam predictions
            con_count += torch.sum((cls_target1 == 0).int())
            con_count += torch.sum((cls_target2 == 0).int())

            # Extract uncontam predictions
            uncon_tmp1 = prediction1[prediction1 == 1]
            uncon_tmp2 = prediction2[prediction2 == 1]

            # GT Labels of uncontam predictions
            target_tmp1 = cls_target1[prediction1 == 1]
            target_tmp2 = cls_target2[prediction2 == 1]

            # Count correct uncontam predictions
            uncon_correct += torch.sum((uncon_tmp1 == target_tmp1).int())
            uncon_correct += torch.sum((uncon_tmp2 == target_tmp2).int())

            # Count the whole number of uncontam predictions
            uncon_count += torch.sum((cls_target1 == 1).int())
            uncon_count += torch.sum((cls_target2 == 1).int())
...
#                import sys; sys.exit()
    ...
    def _get_acc_test_only(self, loader, confusion=False):
        con_count = 0
        con_correct = 0
        uncon_count = 0
        uncon_correct = 0
        ...
        for eval_b_id in pbar:
            ...
            con_tmp = prediction[prediction == 0]
            target_tmp = target[prediction == 0]
            con_correct += torch.sum((con_tmp == target_tmp).int())
            con_count += torch.sum((target == 0).int())

            uncon_tmp = prediction[prediction == 1]
            target_tmp = target[prediction == 1]
            uncon_correct += torch.sum((uncon_tmp == target_tmp).int())
            uncon_count += torch.sum((target == 1).int())

            correct += torch.sum(target == prediction).float().cpu().item()
            accuracy = correct / self.arg.batch_size_test
...
#            ce.append(loss.item())
            ...
            if confusion:
                preds += idx.view(-1).tolist()
                labels += target.view(-1).tolist()

            pbar.set_description_str(desc="[TEST ] Accuracy: {} | Contam: {} | Uncontam: {}".format(
                correct / (eval_b_id+1), con_correct/(con_count+1e-4), uncon_correct/(uncon_count+1e-4))
                , refresh=True
            )
            acc.append(accuracy)

            if con_count == 0 and (eval_b_id + 1) % 500 == 0:
                print("Uncontaminated acc: {}, Vote: {}/{}".format(uncon_correct / uncon_count, uncon_correct, uncon_count))
                uncon_stack.append(uncon_correct)
                uncon_correct = 0; uncon_count = 0

            elif uncon_count == 0 and (eval_b_id + 1) % 500 == 0:
                print("Contaminated acc: {}, Vote: {}/{}".format(con_correct / con_count, con_correct, con_count))
                con_stack.append(con_correct)
                con_correct = 0; con_count = 0

            elif (eval_b_id + 1) % 500 == 0:
                print("Contam acc: {}, Vote:{}/{}\nUncontam acc: {}, Vote:{}/{}".format(
        ...
        if con_count == 0:
            print("Uncontaminated acc: {}".format(uncon_correct / (uncon_count+1e-4)))
        elif uncon_count == 0:
            print("Contaminated acc: {}".format(con_correct / (con_count+1e-4)))
        else:
            print("Contaminated acc: {}, Uncontaminated acc: {}".format(con_correct / (con_count+1e-4), uncon_correct / (uncon_count+1e-4)))
